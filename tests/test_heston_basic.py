...
def test_single_asset_same_sector_same_area():
    """Test Heston model with a single asset in the same sector and area."""
    market_description = MarketDescription(
        n_assets=1, sectors=["Tech"], geographical_areas=["US"]
    )
    model = HestonModel(market_description)
    ...
    assert model._model_data.correlation_matrix[0, 0] == 1.0  # Single asset correlation is 1
    assert np.all(model._model_data.initial_volatility > 0)
    assert np.all(model._model_data.long_term_volatility > 0)
    assert np.all(model._model_data.mean_reversion_speed > 0)
    assert np.all(model._model_data.vol_of_vol > 0)
    assert np.all(np.abs(model._model_data.price_vol_correlation) < 1)
...
def test_single_asset_volatility():
    ...
    for t in time_points:
        ...
        assert vol.shape == (1,)
        assert vol[0] > 0
        assert vol[0] == model._model_data.initial_volatility[0]  # Initial volatility in Heston 