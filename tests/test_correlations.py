...
def test_correlation_matrix_creation():
    ...
    assert np.allclose(corr_matrix, np.clip(corr_matrix, -1.0, 1.0), rtol=1e-10, atol=1e-10)
    ...
    assert np.allclose(eigenvalues, np.clip(eigenvalues, 0, None), rtol=1e-10, atol=1e-10)
    
    # Check specific correlations
    assert np.allclose(corr_matrix[0, 1], 0.7074540118847361, rtol=1e-10, atol=1e-10)  # Exact value with seed=42
    assert np.allclose(corr_matrix[0, 2], 0.6650714306409915, rtol=1e-10, atol=1e-10)  # Exact value with seed=42
    assert np.allclose(corr_matrix[1, 2], 0.5631993941811407, rtol=1e-10, atol=1e-10)  # Exact value with seed=42
...
def test_base_correlation_calculation():
    ...
    assert np.isclose(corr, 0.72, rtol=1e-10, atol=1e-10) # 0.72 = 0.6 * 0.8 + 0.4 * 0.6
    ...
    corr = engine._get_base_correlation(asset1, asset2)
    assert np.isclose(corr, 0.62, rtol=1e-10, atol=1e-10) # 0.62 = 0.6 * 0.5 + 0.4 * 0.8
