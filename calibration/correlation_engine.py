...
class CorrelationEngine:
    ...
    def __init__(self, sector_weight: float = 0.6, geography_weight: float = 0.4, noise_level: float = 0.05):
        """
    ...
    def _get_base_correlation(self, asset1: Asset, asset2: Asset) -> float:
        ...
        sector_key = (min(asset1.sector, asset2.sector), max(asset1.sector, asset2.sector))
        sector_corr = self.SECTOR_CORRELATIONS.get(sector_key, 0.0)
        
        # Get geography correlation
        geo_key = (min(asset1.geography, asset2.geography), max(asset1.geography, asset2.geography))
        geo_corr = self.GEOGRAPHY_CORRELATIONS.get(geo_key, 0.0)
        
        # Combine correlations with weights
        combined_corr = (
            self.sector_weight * sector_corr +
            self.geography_weight * geo_corr
        )
        
        return combined_corr
    
    def create_matrix(self, assets: List[Asset], seed: Optional[int] = None) -> np.ndarray:
        """
